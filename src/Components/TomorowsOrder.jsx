...
console.log(formattedDate);
    ...
    const getTodaysOrder=()=>{
        fetch(`https://ruby-muddy-earthworm.cyclic.app/orders/getorder?filter=${formattedDate}`, {
            method: 'GET',
            headers: {
                Authorization: localStorage.getItem('user_token'),
                'Content-Type': 'application/json',
    ...
    return (
        <Box>{isLoading ? <Image src="https://cdn.dribbble.com/users/645440/screenshots/3266490/loader-2_food.gif" m="auto" />
            ...
            data.length > 0 ? <Box><Text>Tomorows Order</Text>
                ...
                {
                    data.map((e) => (
                        <Box borderRadius="10px" display="flex" w={["90%","70%","70%","60%","60%","60%"]} bg="blue.100" justifyContent="space-around" m="auto" mt="20px" alignItems="center" key={e._id}>
